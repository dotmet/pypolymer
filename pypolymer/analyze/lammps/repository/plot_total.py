...
def main():
    ...
    trjdata = TrjData(fn, LAST_STEP=False)
    data_mat = trjdata.split_by_steps(step_length=1)
    ...
    for wholemol in data_mat:
        ...
        for mol,step in zip(chain, range(1,len(chain)+1)):
            if not ismolInSphere(mol, R=radius, center=[0,0,0]):
                continue
            if iswholeMolInRange(mol, dist, center=[0,0,0], mol_length=10):
                continue
        ...
        theta = np.where(chain_vec[:,1]<0, theta+np.pi, theta)+np.pi/2
        print(theta)
        groups,_l,__l = count_group_1dim(theta, gap=np.pi/14)
        groups_list.append(groups)

        td0 = np.sum(np.array(_l)*np.array(list(map(lambda x:np.mean(x), __l))))/len(_l)
        print(td0)
        param_a.append(td0)
##        print(_, np.sum(_), len(theta))
##        if np.sum(_) != len(theta):
##            print(_,theta)
        #plt.plot([i for i in range(len(theta))], np.sort(theta), marker='o')
##    plt.subplot(121)
##    plt.plot([i for i in range(len(data_mat))], groups_list, marker='o')
##    plt.subplot(122)
    plt.plot([i for i in range(len(param_a))], 1/np.array(param_a), marker='o')
    plt.show()
