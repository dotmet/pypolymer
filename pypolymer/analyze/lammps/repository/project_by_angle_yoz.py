...
def _isCoordOut(mol, axis='x', R=14):
    ...
    _m = mol[:,2:5]
    r = np.abs(np.array([np.linalg.norm(mol[i, 2:5]) for i in range(mol.shape[0])]))
    if max(r)>R:
        return True
    else:
        return False
...
def main():
    ...
    for mol in chain:
        if _isCoordOut(mol, R=radius):
            continue
        #theta = np.arcsin(mol[:,2]/radius)
        z = mol[:,2]
        R = [np.linalg.norm(mol[i,2:5]) for i in range(mol.shape[0])]
        #theta = np.arcsin(mol[:,2]/np.linalg.norm(mol[:,2:5]))
        theta = np.arcsin(z/np.array(R))
        phy = np.arctan(mol[:,4]/mol[:,3])
        phy = np.where(mol[:, 3]<0, np.pi+phy, phy)
        ...
        #phy = (phy-np.pi/2)/np.power(1.2,np.abs(theta))
        if c_id == 0:
